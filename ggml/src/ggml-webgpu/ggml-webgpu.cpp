...
#define WEBGPU_CPU_PROFILE_TOTAL_START(id)                                     \
  auto cpu_total_start_##id = std::chrono::high_resolution_clock::now();

#define WEBGPU_CPU_PROFILE_TOTAL_END(id, ctx)                                  \
  auto cpu_total_end_##id = std::chrono::high_resolution_clock::now();         \
  double cpu_total_time_##id = std::chrono::duration<double, std::milli>(      \
                                   cpu_total_end_##id - cpu_total_start_##id)  \
                                   .count();                                   \
  (ctx)->cpu_time_ms[#id] += cpu_total_time_##id;

// fine-grained timing (not included in totals)
#define WEBGPU_CPU_PROFILE_DETAIL_START(id)                                    \
  auto cpu_detail_start_##id = std::chrono::high_resolution_clock::now();
...
#define WEBGPU_CPU_PROFILE_DETAIL_END(id, ctx)                                 \
  ...
  double cpu_detail_time_##id =                                                \
      std::chrono::duration<double, std::milli>(cpu_detail_end_##id -          \
                                                cpu_detail_start_##id)         \
          .count();                                                            \
...
#define WEBGPU_MAX_INFLIGHT_SUBS_PER_THREAD                                    \
  WEBGPU_NUM_PARAM_BUFS / WEBGPU_COMMAND_SUBMIT_BATCH_SIZE
#define WEBGPU_PARAMS_BUF_SIZE_BYTES 128 // enough for 32 parameters
#define WEBGPU_NUM_SET_ROWS_ERROR_BUFS 32
#define WEBGPU_SET_ROWS_ERROR_BUF_SIZE_BYTES 4
#define WEBGPU_STORAGE_BUF_BINDING_MULT                                        \
  4 // a storage buffer binding size must be a multiple of 4
...
static void ggml_webgpu_create_buffer(wgpu::Device &device,
                                      wgpu::Buffer &buffer, size_t size,
...
struct webgpu_buf_pool {
  ...
  void init(wgpu::Device device, int num_bufs, size_t buf_size,
            wgpu::BufferUsage dev_buf_usage, wgpu::BufferUsage host_buf_usage) {
    for (int i = 0; i < num_bufs; i++) {
      wgpu::Buffer host_buf;
      wgpu::Buffer dev_buf;
      ggml_webgpu_create_buffer(device, host_buf, buf_size, host_buf_usage,
                                "ggml_webgpu_host_pool_buf");
      ggml_webgpu_create_buffer(device, dev_buf, buf_size, dev_buf_usage,
                                "ggml_webgpu_dev_pool_buf");
...
struct webgpu_context_struct {
  ...
  std::unordered_map<ggml_webgpu_mul_mat_pipeline_key, webgpu_pipeline,
  ...
  std::unordered_map<ggml_webgpu_flash_attn_pipeline_key, webgpu_pipeline,
                     ggml_webgpu_flash_attn_pipeline_key_hash>
      flash_attn_pipelines;

  std::unordered_map<int, webgpu_pipeline> argmax_pipelines; // key is vec4
  std::unordered_map<int, webgpu_pipeline>
      argsort_pipelines; // key is order (asc/desc)
  std::unordered_map<int, webgpu_pipeline>
      argsort_merge_pipelines; // key is order (asc/desc)
  std::unordered_map<int, webgpu_pipeline>
      cumsum_pipelines; // key is fixed, no variants yet
  std::unordered_map<int, webgpu_pipeline>
      sum_rows_pipelines; // key is fixed, no variants yet

  std::unordered_map<ggml_webgpu_set_rows_pipeline_key, webgpu_pipeline,
                     ggml_webgpu_set_rows_pipeline_key_hash>
      set_rows_pipelines;
  std::unordered_map<ggml_webgpu_get_rows_pipeline_key, webgpu_pipeline,
                     ggml_webgpu_get_rows_pipeline_key_hash>
      get_rows_pipelines; // src_type, vectorized

  std::map<int, std::map<int, webgpu_pipeline>>
      cpy_pipelines; // src_type, dst_type
  ...
  std::map<int, std::map<int, std::map<int, webgpu_pipeline>>>
      rope_pipelines; // type, ff, inplace
  std::map<int, std::map<int, std::map<int, webgpu_pipeline>>>
      glu_pipelines; // glu_op, type, split

  std::unordered_map<ggml_webgpu_scale_pipeline_key, webgpu_pipeline,
                     ggml_webgpu_scale_pipeline_key_hash>
      scale_pipelines; // inplace
  std::map<int, std::map<int, std::map<int, webgpu_pipeline>>>
      soft_max_pipelines; // mask_type, has_sink, inplace
  std::unordered_map<ggml_webgpu_unary_pipeline_key, webgpu_pipeline,
                     ggml_webgpu_unary_pipeline_key_hash>
      unary_pipelines;
  std::unordered_map<ggml_webgpu_pad_pipeline_key, webgpu_pipeline,
...
struct ggml_backend_webgpu_buffer_context {
  ...
  ggml_backend_webgpu_buffer_context(webgpu_context ctx, wgpu::Buffer buf,
                                     std::string lbl)
      : webgpu_ctx(std::move(ctx)), buffer(std::move(buf)),
        label(std::move(lbl)) {}
};
...
static std::string ggml_webgpu_process_shader_repls(
    const char *src, const std::map<std::string, std::string> &repls) {
  if (!src) {
    return std::string();
  }
  std::string s = src;
...
static webgpu_pipeline ggml_webgpu_create_pipeline(
    wgpu::Device &device, const char *shader_code, const char *label,
...
static void
ggml_backend_webgpu_wait(webgpu_context &ctx,
  ...
  uint32_t inflight_max =
      WEBGPU_MAX_INFLIGHT_SUBS_PER_THREAD / std::max(inflight_threads, 1u);
  while (futures.size() >= inflight_max && futures.size() > 0) {
    ctx->instance.WaitAny(futures[0].futures.size(), futures[0].futures.data(),
                          UINT64_MAX);
    futures.erase(futures.begin());
  }
  size_t i = 0;
  while (i < futures.size()) {
    auto waitStatus = ctx->instance.WaitAny(
        futures[i].futures.size(), futures[i].futures.data(), timeout_ms);
    switch (waitStatus) {
...
static void ggml_backend_webgpu_map_buffer(webgpu_context &ctx,
                                           wgpu::Buffer &buffer,
                                           wgpu::MapMode mode, size_t offset,
                                           size_t size) {
...
static void ggml_backend_webgpu_debug(webgpu_context &ctx) {
  ...
  encoder.CopyBufferToBuffer(ctx->debug_dev_buf, 0, ctx->debug_host_buf, 0,
                             ctx->debug_host_buf.GetSize());
  wgpu::CommandBuffer commands = encoder.Finish();
  ctx->queue.Submit(1, &commands);
  ggml_backend_webgpu_map_buffer(ctx, ctx->debug_host_buf, wgpu::MapMode::Read,
                                 0, ctx->debug_host_buf.GetSize());
  const float *debug_data =
      (const float *)ctx->debug_host_buf.GetConstMappedRange();
  std::cout << "debug[0]: " << debug_data[0] << "\n";
  ctx->debug_host_buf.Unmap();
}
#endif

static webgpu_submission_futures
ggml_backend_webgpu_submit(webgpu_context ctx,
                           std::vector<webgpu_command> commands) {
  std::vector<wgpu::CommandBuffer> command_buffers;
  std::vector<webgpu_pool_bufs> params_bufs;
  std::vector<webgpu_pool_bufs> set_rows_error_bufs;
#ifdef GGML_WEBGPU_GPU_PROFILE
  std::vector<std::pair<std::string, webgpu_gpu_profile_bufs>>
      pipeline_name_and_ts_bufs;
#endif

  for (const auto &command : commands) {
    command_buffers.push_back(command.commands);
    params_bufs.insert(params_bufs.end(), command.params_bufs.begin(),
                       command.params_bufs.end());
  ...
  wgpu::Future p_f = ctx->queue.OnSubmittedWorkDone(
      ...
      [ctx, params_bufs](wgpu::QueueWorkDoneStatus status,
  ...
  for (const auto &bufs : set_rows_error_bufs) {
    wgpu::Future f = bufs.host_buf.MapAsync(
        wgpu::MapMode::Read, 0, bufs.host_buf.GetSize(),
        wgpu::CallbackMode::AllowSpontaneous,
        [ctx, bufs](wgpu::MapAsyncStatus status, wgpu::StringView message) {
          if (status != wgpu::MapAsyncStatus::Success) {
            ...
            const uint32_t *error_data =
                (const uint32_t *)bufs.host_buf.GetConstMappedRange();
            if (*error_data) {
  ...
  for (const auto &command : commands) {
    ...
    wgpu::Future f = ts_bufs.host_buf.MapAsync(
        wgpu::MapMode::Read, 0, ts_bufs.host_buf.GetSize(),
        wgpu::CallbackMode::AllowSpontaneous,
        [ctx, ts_bufs, label](wgpu::MapAsyncStatus status,
                              wgpu::StringView message) {
          if (status != wgpu::MapAsyncStatus::Success) {
            ...
            const uint64_t *ts_data =
                (const uint64_t *)ts_bufs.host_buf.GetConstMappedRange();
...
static webgpu_command ggml_backend_webgpu_build_multi(
    webgpu_context &ctx, const std::vector<webgpu_pipeline> &pipelines,
    const std::vector<std::vector<uint32_t>> &params_list,
    const std::vector<std::vector<wgpu::BindGroupEntry>>
        &bind_group_entries_list,
  ...
  for (size_t i = 0; i < pipelines.size(); i++) {
    ...
    ggml_backend_webgpu_map_buffer(ctx, params_bufs.host_buf,
                                   wgpu::MapMode::Write, 0,
  ...
  for (const auto &params_bufs : params_bufs_list) {
    encoder.CopyBufferToBuffer(params_bufs.host_buf, 0, params_bufs.dev_buf, 0,
  ...
  if (set_rows_error_bufs) {
    encoder.CopyBufferToBuffer(set_rows_error_bufs->dev_buf, 0,
                               set_rows_error_bufs->host_buf, 0,
  ...
  for (size_t i = 0; i < pipelines.size(); i++) {
    ...
    pass.DispatchWorkgroups(workgroups_list[i].first, workgroups_list[i].second,
                            1);
  }
  pass.End();
  ...
  encoder.CopyBufferToBuffer(ts_bufs.dev_buf, 0, ts_bufs.host_buf, 0,
...
static webgpu_command ggml_backend_webgpu_build(
    webgpu_context &ctx, webgpu_pipeline &pipeline,
    std::vector<uint32_t> params,
    std::vector<wgpu::BindGroupEntry> bind_group_entries, uint32_t wg_x,
    uint32_t wg_y = 1,
    std::optional<webgpu_pool_bufs> set_rows_error_bufs = std::nullopt) {
  return ggml_backend_webgpu_build_multi(ctx, {pipeline}, {params},
                                         {bind_group_entries}, {{wg_x, wg_y}},
                                         set_rows_error_bufs);
}

static void ggml_backend_webgpu_buffer_memset(webgpu_context &ctx,
                                              wgpu::Buffer &buf, uint32_t value,
  ...
  webgpu_command command = ggml_backend_webgpu_build(
      ctx, ctx->memset_pipelines[0], params, entries, wg_x);
  std::vector<webgpu_submission_futures> futures = {
      ggml_backend_webgpu_submit(ctx, {command})};
...
static const char *ggml_backend_webgpu_name(ggml_backend_t backend) {
  ggml_backend_webgpu_context *ctx =
      (ggml_backend_webgpu_context *)backend->context;
...
static void ggml_backend_webgpu_free(ggml_backend_t backend) {
  ...
  for (const auto &kv : ctx->webgpu_ctx->cpu_time_ms) {
    ...
    std::cout << "ggml_webgpu:  " << kv.first << ": " << kv.second << " ms ("
              << pct << "%)\n";
  }
  ...
  std::cout << "ggml_webgpu: total gpu time (all shaders): " << total_gpu
            << " ms\n";
  ...
  std::cout << "ggml_webgpu: gpu/cpu ratio: "
            << (total_cpu > 0.0 ? total_gpu / total_cpu : 0.0) << "\n";
#endif
...
static wgpu::Buffer ggml_webgpu_tensor_buf(const ggml_tensor *tensor) {
  ggml_backend_webgpu_buffer_context *ctx =
      (ggml_backend_webgpu_buffer_context *)tensor->buffer->context;
  return ctx->buffer;
}

static size_t ggml_webgpu_tensor_misalignment(webgpu_context &ctx,
                                              const ggml_tensor *t) {
  size_t offset = ggml_webgpu_tensor_offset(t);
  return offset & (ctx->limits.minStorageBufferOffsetAlignment - 1);
}

static size_t ggml_webgpu_tensor_align_offset(webgpu_context &ctx,
                                              const ggml_tensor *t) {
  size_t offset = ggml_webgpu_tensor_offset(t);
  return offset & ~(ctx->limits.minStorageBufferOffsetAlignment - 1);
}

static size_t ggml_webgpu_tensor_binding_size(webgpu_context &ctx,
                                              ggml_tensor *t) {
  return ROUNDUP_POW2(ggml_nbytes(t) + ggml_webgpu_tensor_misalignment(ctx, t),
...
static webgpu_command ggml_webgpu_cpy(webgpu_context &ctx, ggml_tensor *src,
                                      ggml_tensor *dst) {
  uint32_t ne = (uint32_t)ggml_nelements(dst);

  std::vector<uint32_t> params = {
      ne,
      (uint32_t)(ggml_webgpu_tensor_misalignment(ctx, src) /
  ...
  return ggml_backend_webgpu_build(
      ctx, ctx->cpy_pipelines[src->type][dst->type], params, entries, wg_x);
}

static webgpu_command ggml_webgpu_pad(webgpu_context &ctx, ggml_tensor *src,
                                      ggml_tensor *dst) {
  ...
  webgpu_pipeline pipeline;
  {
    ...
    if (it != ctx->pad_pipelines.end()) {
      ...
      pipeline = ggml_webgpu_create_pipeline(
          ctx->device, processed.wgsl.c_str(), processed.variant.c_str());
...
static std::optional<webgpu_command> ggml_webgpu_set_rows(webgpu_context &ctx,
  ...
  ggml_webgpu_set_rows_pipeline_key key = {.dst_type = dst->type,
                                           .vec4 = src->ne[0] % 4 == 0,
                                           .i64_idx =
                                               idx->type == GGML_TYPE_I64};

  ggml_webgpu_set_rows_shader_lib_context shader_lib_ctx = {
      .key = key, .max_wg_size = ctx->limits.maxComputeInvocationsPerWorkgroup};
  ...
  // TODO: remove guard once pipeline caches are per-thread
  {
    ...
    if (it != ctx->set_rows_pipelines.end()) {
      ...
      ggml_webgpu_processed_shader processed =
          ggml_webgpu_preprocess_set_rows_shader(ctx->p, wgsl_set_rows,
                                                 shader_lib_ctx);
      pipeline = ggml_webgpu_create_pipeline(
          ctx->device, processed.wgsl.c_str(), processed.variant.c_str());
      pipeline.context = processed.decisions;
  ...
  std::vector<uint32_t> params = {
      (uint32_t)(ggml_webgpu_tensor_misalignment(ctx, src) /
                 ggml_type_size(src->type)),
      (uint32_t)(ggml_webgpu_tensor_misalignment(ctx, idx) /
                 ggml_type_size(idx->type)),
      (uint32_t)(ggml_webgpu_tensor_misalignment(ctx, dst) /
                 ggml_type_size(dst->type)),
      // Convert byte-strides to element-strides
      (uint32_t)(src->nb[1] / ggml_type_size(src->type)),
  ...
  if (key.i64_idx) {
    entries.push_back({.binding = 3,
                       .buffer = error_bufs->dev_buf,
                       .offset = 0,
                       .size = error_bufs->dev_buf.GetSize()});
  }
...
static std::vector<wgpu::ConstantEntry>
ggml_webgpu_wg_size_entry(uint32_t wg_size) {
...
static webgpu_command ggml_webgpu_get_rows(webgpu_context &ctx,
  ...
  // Create pipeline key
  ggml_webgpu_get_rows_pipeline_key key = {.src_type = src->type,
  ...
  webgpu_pipeline pipeline;
  {
    ...
    if (it != ctx->get_rows_pipelines.end()) {
      ...
      ggml_webgpu_processed_shader processed =
          ggml_webgpu_preprocess_get_rows_shader(ctx->p, shader_src,
                                                 shader_lib_ctx);

      std::vector<wgpu::ConstantEntry> constants =
          ggml_webgpu_wg_size_entry(shader_lib_ctx.max_wg_size);
      pipeline =
  ...
  uint32_t wg_x =
      CEIL_DIV(dst->ne[1] * dst->ne[2] * dst->ne[3], decisions.wg_size);
...
static webgpu_command ggml_webgpu_mul_mat(webgpu_context &ctx,
  ...
  if (use_fast) {
    vectorized =
        (src0->ne[0] % 4 == 0 && dst->ne[0] % 4 == 0 && dst->ne[1] % 4 == 0);
    if (is_vec) {
  ...
  ggml_webgpu_mul_mat_pipeline_key key = {
      ...
      .use_subgroup_matrix =
          (use_fast && !is_vec && ctx->supports_subgroup_matrix) ? 1 : 0,
      .register_tile =
          (use_fast && !is_vec && !ctx->supports_subgroup_matrix) ? 1 : 0};

  // Build shader context
  ggml_webgpu_mul_mat_shader_lib_context shader_lib_ctx = {
      .key = key,
...

  {
    ...
    if (it != ctx->mul_mat_pipelines.end()) {
      ...
      if (shader_src) {
        ggml_webgpu_processed_shader processed =
            ggml_webgpu_preprocess_mul_mat_shader(ctx->p, shader_src,
                                                  shader_lib_ctx);
  ...
  ggml_webgpu_mul_mat_shader_decisions decisions =
      *static_cast<ggml_webgpu_mul_mat_shader_decisions *>(pipeline.context);

  // Build params
  std::vector<uint32_t> params = {
      (uint32_t)(ggml_webgpu_tensor_misalignment(ctx, src0) /
                 ggml_type_size(src0->type)),
      (uint32_t)(ggml_webgpu_tensor_misalignment(ctx, src1) /
...
static webgpu_command ggml_webgpu_flash_attn(webgpu_context &ctx,
  ...
  std::vector<uint32_t> params = {
      (uint32_t)(ggml_webgpu_tensor_misalignment(ctx, Q) /
                 ggml_type_size(Q->type)),
      (uint32_t)(ggml_webgpu_tensor_misalignment(ctx, K) /
                 ggml_type_size(K->type)),
      (uint32_t)(ggml_webgpu_tensor_misalignment(ctx, V) /
                 ggml_type_size(V->type)),
      has_mask ? (uint32_t)(ggml_webgpu_tensor_misalignment(ctx, mask) /
                            ggml_type_size(mask->type))
               : 0,
      has_sinks ? (uint32_t)(ggml_webgpu_tensor_misalignment(ctx, sinks) /
                             ggml_type_size(sinks->type))
                : 0,
      (uint32_t)(ggml_webgpu_tensor_misalignment(ctx, dst) /
                 ggml_type_size(dst->type)),
      ...
      (uint32_t)(Q->nb[1] /
                 ggml_type_size(
                     Q->type)), // stride (elements/blocks) of Q in dimension 1
      (uint32_t)(Q->nb[2] /
                 ggml_type_size(
                     Q->type)), // stride (elements/blocks) of Q in dimension 2
      (uint32_t)(Q->nb[3] /
                 ggml_type_size(
                     Q->type)), // stride (elements/blocks) of Q in dimension 3
      (uint32_t)(K->nb[1] /
                 ggml_type_size(
                     K->type)), // stride (elements/blocks) of K in dimension 1
      (uint32_t)(K->nb[2] /
                 ggml_type_size(
                     K->type)), // stride (elements/blocks) of K in dimension 2
      (uint32_t)(K->nb[3] /
                 ggml_type_size(
                     K->type)), // stride (elements/blocks) of K in dimension 3
      (uint32_t)(V->nb[1] /
                 ggml_type_size(
                     V->type)), // stride (elements/blocks) of V in dimension 1
      (uint32_t)(V->nb[2] /
                 ggml_type_size(
                     V->type)), // stride (elements/blocks) of V in dimension 2
      (uint32_t)(V->nb[3] /
                 ggml_type_size(
                     V->type)), // stride (elements/blocks) of V in dimension 3
      has_mask ? (uint32_t)(mask->nb[3] / ggml_type_size(mask->type))
               : 0, // stride of mask dim 3
      (uint32_t)(Q->ne[2] /
                 K->ne[2]), // repeat factor for K/V in dim 2 (MHA/MQA/GQA)
  ...
  // TODO: remove guard once pipeline caches are per-thread
  {
    ...
    if (it != ctx->flash_attn_pipelines.end()) {
      ...
      ggml_webgpu_processed_shader processed =
          ggml_webgpu_preprocess_flash_attn_shader(ctx->p, wgsl_flash_attn,
                                                   shader_lib_ctx);
      pipeline = ggml_webgpu_create_pipeline(
          ctx->device, processed.wgsl.c_str(), processed.variant.c_str());
      pipeline.context = processed.decisions;
  ...
  uint32_t wg_x = wg_per_head * Q->ne[2] *
                  Q->ne[3]; // wg per head * number of heads * number of batches
  return ggml_backend_webgpu_build(ctx, pipeline, params, entries, wg_x);
}

static webgpu_command ggml_webgpu_unary_op(webgpu_context &ctx,
                                           ggml_tensor *src, ggml_tensor *dst) {
  bool is_unary = dst->op == GGML_OP_UNARY;
  bool inplace =
      ggml_webgpu_tensor_equal(src, dst) || (dst->op == GGML_OP_FILL);
  ...
  webgpu_pipeline pipeline;
  {
    ...
    if (it != ctx->unary_pipelines.end()) {
      ...
      ggml_webgpu_processed_shader processed =
          ggml_webgpu_preprocess_unary_shader(ctx->p, wgsl_unary,
                                              shader_lib_ctx);
      pipeline = ggml_webgpu_create_pipeline(
          ctx->device, processed.wgsl.c_str(), processed.variant.c_str());
      pipeline.context = processed.decisions;
...
static webgpu_command ggml_webgpu_rms_norm(webgpu_context &ctx,
                                           ggml_tensor *src, ggml_tensor *dst) {
  ...
  return ggml_backend_webgpu_build(ctx, ctx->rms_norm_pipelines[inplace],
                                   params, entries, ggml_nrows(src));
}

static webgpu_command ggml_webgpu_rope(webgpu_context &ctx, ggml_tensor *src0,
                                       ggml_tensor *src1, ggml_tensor *src2,
                                       ggml_tensor *dst) {
  ...
  float corr_dims[2];
  ggml_rope_yarn_corr_dims(n_dims, n_ctx_orig, freq_base, beta_fast, beta_slow,
                           corr_dims);
  ...
  std::vector<uint32_t> params = {
      ...
      (uint32_t)(ggml_webgpu_tensor_misalignment(ctx, src1) /
                 ggml_type_size(src1->type)),
      src2 != nullptr ? (uint32_t)(ggml_webgpu_tensor_misalignment(ctx, src2) /
  ...
  webgpu_pipeline pipeline =
      ctx->rope_pipelines[dst->type][has_freq_factor][inplace];
  uint32_t wg_x = CEIL_DIV(ggml_nelements(dst), WEBGPU_MAX_WG_SIZE);
  return ggml_backend_webgpu_build(ctx, pipeline, params, entries, wg_x);
}

static webgpu_command ggml_webgpu_glu(webgpu_context &ctx, ggml_tensor *src0,
                                      ggml_tensor *src1, ggml_tensor *dst) {
  ...
  std::vector<uint32_t> params = {
      (uint32_t)(ggml_webgpu_tensor_misalignment(ctx, src0) /
                 ggml_type_size(src0->type)),
      src1 != nullptr ? (uint32_t)(ggml_webgpu_tensor_misalignment(ctx, src1) /
                                   ggml_type_size(src1->type))
                      : 0,
      (uint32_t)(ggml_webgpu_tensor_misalignment(ctx, dst) /
                 ggml_type_size(dst->type)),
  ...
  webgpu_pipeline pipeline =
      ctx->glu_pipelines[ggml_get_glu_op(dst)][dst->type][split];
  uint32_t wg_x = CEIL_DIV(ggml_nelements(dst), WEBGPU_MAX_WG_SIZE);
  return ggml_backend_webgpu_build(ctx, pipeline, params, entries, wg_x);
}

static webgpu_command ggml_webgpu_scale(webgpu_context &ctx, ggml_tensor *src,
                                        ggml_tensor *dst) {
  int inplace = ggml_webgpu_tensor_equal(src, dst);
  ...
  // TODO: remove guard once pipeline caches are per-thread
  {
    ...
    if (it != ctx->scale_pipelines.end()) {
      ...
      ggml_webgpu_processed_shader processed =
          ggml_webgpu_preprocess_scale_shader(ctx->p, wgsl_scale,
                                              shader_lib_ctx);
      pipeline = ggml_webgpu_create_pipeline(
          ctx->device, processed.wgsl.c_str(), processed.variant.c_str());
      pipeline.context = processed.decisions;
...
static webgpu_command ggml_webgpu_soft_max(webgpu_context &ctx,
  ...
  const int inplace = ggml_webgpu_tensor_equal(src0, dst);
  const int mask_type =
      (src1 != nullptr) ? src1->type : 2; // use 2 for no mask here
  ...
  return ggml_backend_webgpu_build(
      ctx, ctx->soft_max_pipelines[mask_type][has_sink][inplace], params,
      entries, ggml_nrows(dst));
}

static webgpu_command ggml_webgpu_argmax(webgpu_context &ctx, ggml_tensor *src,
                                         ggml_tensor *dst) {
  std::vector<uint32_t> params = {
      (uint32_t)(ggml_webgpu_tensor_misalignment(ctx, src) /
                 ggml_type_size(src->type)),
  ...
  webgpu_pipeline pipeline;
  {
    ...
    if (it != ctx->argmax_pipelines.end()) {
      ...
      ggml_webgpu_processed_shader processed =
          ggml_webgpu_preprocess_generic_shader(ctx->p, wgsl_argmax,
                                                shader_lib_ctx, "argmax");
      pipeline = ggml_webgpu_create_pipeline(
          ctx->device, processed.wgsl.c_str(), processed.variant.c_str());
...
static webgpu_command ggml_webgpu_argsort(webgpu_context &ctx, ggml_tensor *src,
                                          ggml_tensor *dst) {
  bool is_top_k = dst->op == GGML_OP_TOP_K;
  // ascending order is 0, descending order is 1
  const int32_t order = is_top_k ? (int32_t)GGML_SORT_ORDER_DESC
                                 : (int32_t)ggml_get_op_params_i32(dst, 0);
  ...
  if (it != ctx->argsort_pipelines.end()) {
    ...
    ggml_webgpu_processed_shader processed =
        ggml_webgpu_preprocess_argsort_shader(ctx->p, wgsl_argsort,
                                              shader_lib_ctx);
    argsort_pipeline = ggml_webgpu_create_pipeline(
        ctx->device, processed.wgsl.c_str(), processed.variant.c_str());
  ...
  ggml_webgpu_argsort_shader_decisions argsort_decisions =
      *static_cast<ggml_webgpu_argsort_shader_decisions *>(
          argsort_pipeline.context);
  ...
  if (it != ctx->argsort_merge_pipelines.end()) {
    ...
    ggml_webgpu_processed_shader processed =
        ggml_webgpu_preprocess_argsort_merge_shader(ctx->p, wgsl_argsort_merge,
                                                    shader_lib_ctx);
    argsort_merge_pipeline = ggml_webgpu_create_pipeline(
        ctx->device, processed.wgsl.c_str(), processed.variant.c_str());
  ...
  const uint32_t block_size =
      is_top_k ? std::min(argsort_decisions.wg_size, (uint32_t)dst->ne[0])
               : argsort_decisions.wg_size;
  uint32_t out_ne0 = src_ne0;
  if (is_top_k) {
    if (npr > 1) {
      const uint32_t last_tile =
          src_ne0 - (npr - 1) * argsort_decisions.wg_size;
  ...
  const size_t tmp_offset = ROUNDUP_POW2(
      dst_offset + idx_nbytes, ctx->limits.minStorageBufferOffsetAlignment);
  const size_t tmp_binding_size =
      ROUNDUP_POW2(idx_nbytes, WEBGPU_STORAGE_BUF_BINDING_MULT);
  const size_t dst_binding_size =
      ROUNDUP_POW2(idx_nbytes + ggml_webgpu_tensor_misalignment(ctx, dst),
                   WEBGPU_STORAGE_BUF_BINDING_MULT);

  const uint32_t offset_src =
      (uint32_t)(ggml_webgpu_tensor_misalignment(ctx, src) /
                 ggml_type_size(src->type));
  const uint32_t offset_dst =
      (uint32_t)(ggml_webgpu_tensor_misalignment(ctx, dst) /
                 ggml_type_size(dst->type));
  const uint32_t offset_tmp = 0;
  const uint32_t stride_src1 =
      (uint32_t)(src->nb[1] / ggml_type_size(src->type));
  const uint32_t stride_src2 =
      (uint32_t)(src->nb[2] / ggml_type_size(src->type));
  const uint32_t stride_src3 =
      (uint32_t)(src->nb[3] / ggml_type_size(src->type));
  ...
  const size_t init_align_offset =
      start_in_tmp ? tmp_offset : ggml_webgpu_tensor_align_offset(ctx, dst);
  const size_t init_binding_size =
      start_in_tmp ? tmp_binding_size : dst_binding_size;

  std::vector<uint32_t> init_params = {offset_src,
                                       init_offset,
                                       stride_src1,
                                       stride_src2,
                                       stride_src3,
  ...
  std::vector<wgpu::BindGroupEntry> init_entries = {
      ...
      {.binding = 1,
       .buffer = ggml_webgpu_tensor_buf(dst),
       .offset = init_align_offset,
       .size = init_binding_size}};
  ...
  if (merge_passes == 0) {
    return ggml_backend_webgpu_build_multi(ctx, pipelines, params_list,
  ...
  while (len < out_ne0) {
    ...
    const size_t align_in =
        in_is_tmp ? tmp_offset : ggml_webgpu_tensor_align_offset(ctx, dst);
    const size_t align_out =
        out_is_tmp ? tmp_offset : ggml_webgpu_tensor_align_offset(ctx, dst);
    ...
    const uint32_t top_k_out =
        (is_top_k && nm == 1) ? (uint32_t)dst->ne[0] : out_ne0;
...
static webgpu_command ggml_webgpu_cumsum(webgpu_context &ctx, ggml_tensor *src,
                                         ggml_tensor *dst) {
  std::vector<uint32_t> params = {
      (uint32_t)(ggml_webgpu_tensor_misalignment(ctx, src) /
                 ggml_type_size(src->type)),
  ...
  // TODO: remove guard once pipeline caches are per-thread
  {
    ...
    if (it != ctx->cumsum_pipelines.end()) {
      ...
      ggml_webgpu_processed_shader processed =
          ggml_webgpu_preprocess_generic_shader(ctx->p, wgsl_cumsum,
                                                shader_lib_ctx, "cumsum");
      pipeline = ggml_webgpu_create_pipeline(
          ctx->device, processed.wgsl.c_str(), processed.variant.c_str());
...
static webgpu_command ggml_webgpu_sum_rows(webgpu_context &ctx,
                                           ggml_tensor *src, ggml_tensor *dst) {
  ...
  std::vector<uint32_t> params = {
      ...
      total_sum ? static_cast<uint32_t>(ggml_nelements(src))
                : (uint32_t)src->ne[0],
  ...
  webgpu_pipeline pipeline;
  {
    ...
    if (it != ctx->sum_rows_pipelines.end()) {
      ...
      ggml_webgpu_processed_shader processed =
          ggml_webgpu_preprocess_generic_shader(ctx->p, wgsl_sum_rows,
                                                shader_lib_ctx, "sum_rows");
      pipeline = ggml_webgpu_create_pipeline(
          ctx->device, processed.wgsl.c_str(), processed.variant.c_str());
...
static std::optional<webgpu_command>
ggml_webgpu_encode_node(webgpu_context ctx, ggml_tensor *node) {
  ...
  WEBGPU_LOG_DEBUG("ggml_webgpu_encode_node(" << node << ", "
  ...
  case GGML_OP_FLASH_ATTN_EXT:
    return ggml_webgpu_flash_attn(ctx, src0, src1, src2, node->src[3],
                                  node->src[4], node);
  case GGML_OP_ADD: {
    int inplace = ggml_webgpu_tensor_equal(src0, node);
    return ggml_webgpu_binary_op(ctx, src0, src1, node,
                                 ctx->add_pipelines[node->type][inplace],
                                 inplace);
  }
  case GGML_OP_SUB: {
    int inplace = ggml_webgpu_tensor_equal(src0, node);
    return ggml_webgpu_binary_op(ctx, src0, src1, node,
                                 ctx->sub_pipelines[node->type][inplace],
                                 inplace);
  }
  case GGML_OP_MUL: {
    int inplace = ggml_webgpu_tensor_equal(src0, node);
    return ggml_webgpu_binary_op(ctx, src0, src1, node,
                                 ctx->mul_pipelines[node->type][inplace],
                                 inplace);
  }
  case GGML_OP_DIV: {
    int inplace = ggml_webgpu_tensor_equal(src0, node);
    return ggml_webgpu_binary_op(ctx, src0, src1, node,
                                 ctx->div_pipelines[node->type][inplace],
                                 inplace);
  }
...
static ggml_status
ggml_backend_webgpu_graph_compute(ggml_backend_t backend,
                                  struct ggml_cgraph *cgraph) {
  WEBGPU_LOG_DEBUG("ggml_backend_webgpu_graph_compute(" << cgraph->n_nodes
                                                        << " nodes)");

  ggml_backend_webgpu_context *backend_ctx =
      static_cast<ggml_backend_webgpu_context *>(backend->context);
  ...
  for (int i = 0; i < cgraph->n_nodes; i++) {
    ...
    uint32_t batch_size = std::min(
        std::max(1u, WEBGPU_NUM_PARAM_BUFS / std::max(inflight_threads, 1u)),
  ...
  if (!commands.empty()) {
    webgpu_submission_futures new_futures =
        ggml_backend_webgpu_submit(ctx, commands);
...
static void
ggml_backend_webgpu_buffer_free_buffer(ggml_backend_buffer_t buffer) {
  ggml_backend_webgpu_buffer_context *ctx =
      static_cast<ggml_backend_webgpu_buffer_context *>(buffer->context);
  ctx->buffer.Destroy();
}
...
static void
ggml_backend_webgpu_buffer_memset_tensor(ggml_backend_buffer_t buffer,
                                         ggml_tensor *tensor, uint8_t value,
                                         size_t offset, size_t size) {
  if (size == 0) {
    WEBGPU_LOG_DEBUG("ggml_backend_webgpu_buffer_memset_tensor: size is zero, "
                     "nothing to do.");
    return;
  }
  ...
  ggml_backend_webgpu_buffer_context *buf_ctx =
      (ggml_backend_webgpu_buffer_context *)buffer->context;

  WEBGPU_LOG_DEBUG("ggml_backend_webgpu_buffer_memset_tensor("
                   << buf_ctx->label << ", " << tensor << ", " << value << ", "
                   << offset << ", " << size << ")");

  size_t total_offset =
      webgpu_tensor_offset(tensor) + tensor->view_offs + offset;

  // This is a trick to set all bytes of a u32 to the same 1 byte value.
  uint32_t val32 = (uint32_t)value * 0x01010101;
  ggml_backend_webgpu_buffer_memset(buf_ctx->webgpu_ctx, buf_ctx->buffer, val32,
                                    total_offset, size);
...
static void ggml_backend_webgpu_buffer_set_tensor(ggml_backend_buffer_t buffer,
  ...
  WEBGPU_LOG_DEBUG("ggml_backend_webgpu_buffer_set_tensor("
                   << buf_ctx->label << ", " << tensor << ", " << data << ", "
                   << offset << ", " << size << ")");

  size_t total_offset =
      webgpu_tensor_offset(tensor) + tensor->view_offs + offset;

  webgpu_ctx->queue.WriteBuffer(buf_ctx->buffer, total_offset, data,
                                (size / 4) * 4);
  ...
  if (size % 4 != 0) {
    ...
    for (size_t i = 0; i < remaining_size; i++) {
      ((uint8_t *)&val32)[i] =
          ((const uint8_t *)data)[size - remaining_size + i];
    }
    // memset the remaining bytes
    ggml_backend_webgpu_buffer_memset(webgpu_ctx, buf_ctx->buffer, val32,
                                      total_offset + (size - remaining_size),
                                      remaining_size);
  } else {
    // wait for WriteBuffer to complete
    webgpu_ctx->instance.WaitAny(
        webgpu_ctx->queue.OnSubmittedWorkDone(
            wgpu::CallbackMode::AllowSpontaneous,
...
static void ggml_backend_webgpu_buffer_get_tensor(ggml_backend_buffer_t buffer,
  ...
  ggml_backend_webgpu_buffer_context *buf_ctx =
      (ggml_backend_webgpu_buffer_context *)buffer->context;
  WEBGPU_LOG_DEBUG("ggml_backend_webgpu_buffer_get_tensor("
                   << buf_ctx->label << ", " << tensor << ", " << data << ", "
                   << offset << ", " << size << ")");
  webgpu_context webgpu_ctx = buf_ctx->webgpu_ctx;
  ...
  if (webgpu_ctx->get_tensor_staging_buf == nullptr ||
    ...
    ggml_webgpu_create_buffer(
        device, webgpu_ctx->get_tensor_staging_buf, final_size,
  ...
  encoder.CopyBufferToBuffer(buf_ctx->buffer, total_offset,
                             webgpu_ctx->get_tensor_staging_buf, 0, final_size);
  wgpu::CommandBuffer commands = encoder.Finish();
  ...
  ggml_backend_webgpu_map_buffer(webgpu_ctx, webgpu_ctx->get_tensor_staging_buf,
  ...
  const void *mapped_range =
      webgpu_ctx->get_tensor_staging_buf.GetConstMappedRange(0, final_size);
...
static void ggml_backend_webgpu_buffer_clear(ggml_backend_buffer_t buffer,
                                             uint8_t value) {
  WEBGPU_LOG_DEBUG("ggml_backend_webgpu_buffer_clear("
                   << buffer << ", " << (uint32_t)value << ")");
  WEBGPU_CPU_PROFILE_TOTAL_START(clear);
  ggml_backend_webgpu_buffer_context *buf_ctx =
      (ggml_backend_webgpu_buffer_context *)buffer->context;
  ggml_backend_webgpu_buffer_memset(buf_ctx->webgpu_ctx, buf_ctx->buffer, value,
                                    0, buffer->size);
...
static const char *
ggml_backend_webgpu_buffer_type_get_name(ggml_backend_buffer_type_t buft) {
  ggml_backend_webgpu_device_context *ctx =
      static_cast<ggml_backend_webgpu_device_context *>(buft->device->context);
  return ctx->device_name.c_str();
}

static ggml_backend_buffer_t
ggml_backend_webgpu_buffer_type_alloc_buffer(ggml_backend_buffer_type_t buft,
                                             size_t size) {
  static std::atomic<int> buffer_count;
  int buffer_id = buffer_count++;
  std::string buf_name = "tensor_buf" + std::to_string(buffer_id);
  WEBGPU_LOG_DEBUG("ggml_backend_webgpu_buffer_type_alloc_buffer_"
                   << buffer_id << ": " << size << " bytes");
  ggml_backend_webgpu_device_context *ctx =
      static_cast<ggml_backend_webgpu_device_context *>(buft->device->context);

  wgpu::Buffer buf;
  ggml_webgpu_create_buffer(ctx->webgpu_ctx->device, buf,
                            ROUNDUP_POW2(size, WEBGPU_STORAGE_BUF_BINDING_MULT),
  ...
  return ggml_backend_buffer_init(buft, ggml_backend_webgpu_buffer_interface,
                                  buf_ctx, size);
}

static size_t
ggml_backend_webgpu_buffer_type_get_alignment(ggml_backend_buffer_type_t buft) {
  ggml_backend_webgpu_device_context *ctx =
      static_cast<ggml_backend_webgpu_device_context *>(buft->device->context);
...
static size_t
ggml_backend_webgpu_buffer_type_get_max_size(ggml_backend_buffer_type_t buft) {
  ggml_backend_webgpu_device_context *ctx =
      static_cast<ggml_backend_webgpu_device_context *>(buft->device->context);
...
static size_t
ggml_backend_webgpu_buffer_type_get_alloc_size(ggml_backend_buffer_type_t buft,
                                               const ggml_tensor *tensor) {
  ggml_backend_webgpu_device_context *ctx =
      static_cast<ggml_backend_webgpu_device_context *>(buft->device->context);
  ...
  case GGML_OP_ARGSORT:
    res = ROUNDUP_POW2(
        res * 2 + ctx->webgpu_ctx->limits.minStorageBufferOffsetAlignment,
        WEBGPU_STORAGE_BUF_BINDING_MULT);
    break;
  case GGML_OP_TOP_K: {
    ...
    if (src0) {
      ...
      res = ROUNDUP_POW2(
          full * 2 + ctx->webgpu_ctx->limits.minStorageBufferOffsetAlignment,
          WEBGPU_STORAGE_BUF_BINDING_MULT);
    }
  } break;
  default:
...
static const char *ggml_backend_webgpu_device_get_name(ggml_backend_dev_t dev) {
  ggml_backend_webgpu_device_context *ctx =
      static_cast<ggml_backend_webgpu_device_context *>(dev->context);
  return ctx->device_name.c_str();
}

static const char *
ggml_backend_webgpu_device_get_description(ggml_backend_dev_t dev) {
  ggml_backend_webgpu_device_context *ctx =
      static_cast<ggml_backend_webgpu_device_context *>(dev->context);
  return ctx->device_desc.c_str();
}

static void ggml_backend_webgpu_device_get_memory(ggml_backend_dev_t dev,
...
ggml_backend_webgpu_device_get_type(ggml_backend_dev_t dev) {
  GGML_UNUSED(dev);
  return GGML_BACKEND_DEVICE_TYPE_GPU;
}

static void
ggml_backend_webgpu_device_get_props(ggml_backend_dev_t dev,
  ...
  ggml_backend_webgpu_device_get_memory(dev, &props->memory_free,
                                        &props->memory_total);
  props->caps = {
      /* .async                 = */ false,
      /* .host_buffer           = */ false,
...
static void ggml_webgpu_init_memset_pipeline(webgpu_context &webgpu_ctx) {
  ...
  size_t max_threads =
      WEBGPU_MAX_WG_SIZE * webgpu_ctx->limits.maxComputeWorkgroupsPerDimension;
  // Size the bytes_per_thread so that the largest buffer size can be handled
  webgpu_ctx->memset_bytes_per_thread =
      CEIL_DIV(webgpu_ctx->limits.maxStorageBufferBindingSize, max_threads);
  std::vector<wgpu::ConstantEntry> constants(2);
  constants[0].key = "wg_size";
  constants[0].value = WEBGPU_MAX_WG_SIZE;
  constants[1].key = "bytes_per_thread";
  constants[1].value = webgpu_ctx->memset_bytes_per_thread;
  webgpu_ctx->memset_pipelines[0] = ggml_webgpu_create_pipeline(
      webgpu_ctx->device, wgsl_memset, "memset", constants);
}

static void ggml_webgpu_init_cpy_pipeline(webgpu_context &webgpu_ctx) {
  std::vector<wgpu::ConstantEntry> constants =
      ggml_webgpu_wg_size_entry(WEBGPU_MAX_WG_SIZE);

  webgpu_ctx->cpy_pipelines[GGML_TYPE_F32][GGML_TYPE_F32] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device, wgsl_cpy_f32_f32,
                                  "cpy_f32_f32", constants);
  webgpu_ctx->cpy_pipelines[GGML_TYPE_F32][GGML_TYPE_I32] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device, wgsl_cpy_f32_i32,
                                  "cpy_f32_i32", constants);
  webgpu_ctx->cpy_pipelines[GGML_TYPE_F32][GGML_TYPE_F16] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device, wgsl_cpy_f32_f16,
                                  "cpy_f32_f16", constants);
  webgpu_ctx->cpy_pipelines[GGML_TYPE_F16][GGML_TYPE_F32] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device, wgsl_cpy_f16_f32,
                                  "cpy_f16_f32", constants);
  webgpu_ctx->cpy_pipelines[GGML_TYPE_F16][GGML_TYPE_F16] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device, wgsl_cpy_f16_f16,
                                  "cpy_f16_f16", constants);
}
...
static void ggml_webgpu_init_add_pipeline(webgpu_context &webgpu_ctx) {
  ...
  webgpu_ctx->add_pipelines[GGML_TYPE_F32][0] = ggml_webgpu_create_pipeline(
      webgpu_ctx->device, wgsl_add_f32, "add_f32", constants);
  webgpu_ctx->add_pipelines[GGML_TYPE_F16][0] = ggml_webgpu_create_pipeline(
      webgpu_ctx->device, wgsl_add_f16, "add_f16", constants);
  webgpu_ctx->add_pipelines[GGML_TYPE_F32][1] = ggml_webgpu_create_pipeline(
      webgpu_ctx->device, wgsl_add_f32_inplace, "add_f32_inplace", constants);
  webgpu_ctx->add_pipelines[GGML_TYPE_F16][1] = ggml_webgpu_create_pipeline(
...
static void ggml_webgpu_init_sub_pipeline(webgpu_context &webgpu_ctx) {
  ...
  webgpu_ctx->sub_pipelines[GGML_TYPE_F32][0] = ggml_webgpu_create_pipeline(
      webgpu_ctx->device, wgsl_sub_f32, "sub_f32", constants);
  webgpu_ctx->sub_pipelines[GGML_TYPE_F16][0] = ggml_webgpu_create_pipeline(
      webgpu_ctx->device, wgsl_sub_f16, "sub_f16", constants);
  webgpu_ctx->sub_pipelines[GGML_TYPE_F32][1] = ggml_webgpu_create_pipeline(
      webgpu_ctx->device, wgsl_sub_f32_inplace, "sub_f32_inplace", constants);
  webgpu_ctx->sub_pipelines[GGML_TYPE_F16][1] = ggml_webgpu_create_pipeline(
...
static void ggml_webgpu_init_mul_pipeline(webgpu_context &webgpu_ctx) {
  ...
  webgpu_ctx->mul_pipelines[GGML_TYPE_F32][0] = ggml_webgpu_create_pipeline(
      webgpu_ctx->device, wgsl_mul_f32, "mul_f32", constants);
  webgpu_ctx->mul_pipelines[GGML_TYPE_F16][0] = ggml_webgpu_create_pipeline(
      webgpu_ctx->device, wgsl_mul_f16, "mul_f16", constants);
  webgpu_ctx->mul_pipelines[GGML_TYPE_F32][1] = ggml_webgpu_create_pipeline(
      webgpu_ctx->device, wgsl_mul_f32_inplace, "mul_f32_inplace", constants);
  webgpu_ctx->mul_pipelines[GGML_TYPE_F16][1] = ggml_webgpu_create_pipeline(
...
static void ggml_webgpu_init_div_pipeline(webgpu_context &webgpu_ctx) {
  ...
  webgpu_ctx->div_pipelines[GGML_TYPE_F32][0] = ggml_webgpu_create_pipeline(
      webgpu_ctx->device, wgsl_div_f32, "div_f32", constants);
  webgpu_ctx->div_pipelines[GGML_TYPE_F16][0] = ggml_webgpu_create_pipeline(
      webgpu_ctx->device, wgsl_div_f16, "div_f16", constants);
  webgpu_ctx->div_pipelines[GGML_TYPE_F32][1] = ggml_webgpu_create_pipeline(
      webgpu_ctx->device, wgsl_div_f32_inplace, "div_f32_inplace", constants);
  webgpu_ctx->div_pipelines[GGML_TYPE_F16][1] = ggml_webgpu_create_pipeline(
      webgpu_ctx->device, wgsl_div_f16_inplace, "div_f16_inplace", constants);
}

static void ggml_webgpu_init_rms_norm_pipeline(webgpu_context &webgpu_ctx) {
  std::vector<wgpu::ConstantEntry> constants =
      ggml_webgpu_wg_size_entry(WEBGPU_ROW_SPLIT_WG_SIZE);

  webgpu_ctx->rms_norm_pipelines[0] = ggml_webgpu_create_pipeline(
...
static void ggml_webgpu_init_rope_pipeline(webgpu_context &webgpu_ctx) {
  ...
  webgpu_ctx->rope_pipelines[GGML_TYPE_F32][0][0] = ggml_webgpu_create_pipeline(
  ...
  webgpu_ctx->rope_pipelines[GGML_TYPE_F32][1][0] = ggml_webgpu_create_pipeline(
      webgpu_ctx->device, wgsl_rope_f32_ff, "rope_f32_ff", constants);
  webgpu_ctx->rope_pipelines[GGML_TYPE_F32][1][1] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device, wgsl_rope_f32_ff_inplace,
                                  "rope_f32_ff_inplace", constants);

  webgpu_ctx->rope_pipelines[GGML_TYPE_F16][0][0] = ggml_webgpu_create_pipeline(
  ...
  webgpu_ctx->rope_pipelines[GGML_TYPE_F16][1][0] = ggml_webgpu_create_pipeline(
      webgpu_ctx->device, wgsl_rope_f16_ff, "rope_f16_ff", constants);
  webgpu_ctx->rope_pipelines[GGML_TYPE_F16][1][1] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device, wgsl_rope_f16_ff_inplace,
                                  "rope_f16_ff_inplace", constants);
}
...
static void ggml_webgpu_init_glu_pipeline(webgpu_context &webgpu_ctx) {
  ...
  webgpu_ctx->glu_pipelines[GGML_GLU_OP_REGLU][GGML_TYPE_F32][0] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device, wgsl_reglu_f32,
                                  "reglu_f32", constants);
  webgpu_ctx->glu_pipelines[GGML_GLU_OP_REGLU][GGML_TYPE_F16][0] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device, wgsl_reglu_f16,
                                  "reglu_f16", constants);
  webgpu_ctx->glu_pipelines[GGML_GLU_OP_REGLU][GGML_TYPE_F32][1] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device, wgsl_reglu_f32_split,
                                  "reglu_f32_split", constants);
  webgpu_ctx->glu_pipelines[GGML_GLU_OP_REGLU][GGML_TYPE_F16][1] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device, wgsl_reglu_f16_split,
                                  "reglu_f16_split", constants);

  // GEGLU
  webgpu_ctx->glu_pipelines[GGML_GLU_OP_GEGLU][GGML_TYPE_F32][0] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device, wgsl_geglu_f32,
                                  "geglu_f32", constants);
  webgpu_ctx->glu_pipelines[GGML_GLU_OP_GEGLU][GGML_TYPE_F16][0] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device, wgsl_geglu_f16,
                                  "geglu_f16", constants);
  webgpu_ctx->glu_pipelines[GGML_GLU_OP_GEGLU][GGML_TYPE_F32][1] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device, wgsl_geglu_f32_split,
                                  "geglu_f32_split", constants);
  webgpu_ctx->glu_pipelines[GGML_GLU_OP_GEGLU][GGML_TYPE_F16][1] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device, wgsl_geglu_f16_split,
                                  "geglu_f16_split", constants);

  // SWIGLU
  webgpu_ctx->glu_pipelines[GGML_GLU_OP_SWIGLU][GGML_TYPE_F32][0] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device, wgsl_swiglu_f32,
                                  "swiglu_f32", constants);
  webgpu_ctx->glu_pipelines[GGML_GLU_OP_SWIGLU][GGML_TYPE_F16][0] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device, wgsl_swiglu_f16,
                                  "swiglu_f16", constants);
  webgpu_ctx->glu_pipelines[GGML_GLU_OP_SWIGLU][GGML_TYPE_F32][1] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device, wgsl_swiglu_f32_split,
                                  "swiglu_f32_split", constants);
  webgpu_ctx->glu_pipelines[GGML_GLU_OP_SWIGLU][GGML_TYPE_F16][1] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device, wgsl_swiglu_f16_split,
                                  "swiglu_f16_split", constants);

  // SWIGLU_OAI
  webgpu_ctx->glu_pipelines[GGML_GLU_OP_SWIGLU_OAI][GGML_TYPE_F32][0] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device, wgsl_swiglu_oai_f32,
                                  "swiglu_oai_f32", constants);
  webgpu_ctx->glu_pipelines[GGML_GLU_OP_SWIGLU_OAI][GGML_TYPE_F32][1] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device, wgsl_swiglu_oai_f32_split,
                                  "swiglu_oai_f32_split", constants);

  // GEGLU_ERF
  webgpu_ctx->glu_pipelines[GGML_GLU_OP_GEGLU_ERF][GGML_TYPE_F32][0] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device, wgsl_geglu_erf_f32,
                                  "geglu_erf_f32", constants);
  webgpu_ctx->glu_pipelines[GGML_GLU_OP_GEGLU_ERF][GGML_TYPE_F16][0] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device, wgsl_geglu_erf_f16,
                                  "geglu_erf_f16", constants);
  webgpu_ctx->glu_pipelines[GGML_GLU_OP_GEGLU_ERF][GGML_TYPE_F32][1] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device, wgsl_geglu_erf_f32_split,
                                  "geglu_erf_f32_split", constants);
  webgpu_ctx->glu_pipelines[GGML_GLU_OP_GEGLU_ERF][GGML_TYPE_F16][1] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device, wgsl_geglu_erf_f16_split,
                                  "geglu_erf_f16_split", constants);

  // GEGLU_QUICK
  webgpu_ctx->glu_pipelines[GGML_GLU_OP_GEGLU_QUICK][GGML_TYPE_F32][0] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device, wgsl_geglu_quick_f32,
                                  "geglu_quick_f32", constants);
  webgpu_ctx->glu_pipelines[GGML_GLU_OP_GEGLU_QUICK][GGML_TYPE_F16][0] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device, wgsl_geglu_quick_f16,
                                  "geglu_quick_f16", constants);
  webgpu_ctx->glu_pipelines[GGML_GLU_OP_GEGLU_QUICK][GGML_TYPE_F32][1] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device,
                                  wgsl_geglu_quick_f32_split,
                                  "geglu_quick_f32_split", constants);
  webgpu_ctx->glu_pipelines[GGML_GLU_OP_GEGLU_QUICK][GGML_TYPE_F16][1] =
      ggml_webgpu_create_pipeline(webgpu_ctx->device,
                                  wgsl_geglu_quick_f16_split,
                                  "geglu_quick_f16_split", constants);
}
...
static void ggml_webgpu_init_soft_max_pipeline(webgpu_context &webgpu_ctx) {
  ...
  webgpu_ctx->soft_max_pipelines[1][0][0] = ggml_webgpu_create_pipeline(
      webgpu_ctx->device, wgsl_soft_max_f32_mask_f16, "soft_max_f32_mask_f16",
      constants);
  webgpu_ctx->soft_max_pipelines[1][0][1] = ggml_webgpu_create_pipeline(
      webgpu_ctx->device, wgsl_soft_max_f32_mask_f16_inplace,
      "soft_max_f32_mask_f16_inplace", constants);
  webgpu_ctx->soft_max_pipelines[1][1][0] = ggml_webgpu_create_pipeline(
      webgpu_ctx->device, wgsl_soft_max_f32_mask_f16_sink,
      "soft_max_f32_mask_f16_sink", constants);
  webgpu_ctx->soft_max_pipelines[1][1][1] = ggml_webgpu_create_pipeline(
      webgpu_ctx->device, wgsl_soft_max_f32_mask_f16_sink_inplace,
      "soft_max_f32_mask_f16_sink_inplace", constants);
}

// TODO: move most initialization logic here
static ggml_backend_t ggml_backend_webgpu_device_init(ggml_backend_dev_t dev,
                                                      const char *params) {
  GGML_UNUSED(params);
  ...
  backend_ctx.name =
      GGML_WEBGPU_NAME + std::string(": ") + dev_ctx->device_name;
...
ggml_backend_webgpu_device_get_buffer_type(ggml_backend_dev_t dev) {
...
static bool
ggml_backend_webgpu_device_supports_buft(ggml_backend_dev_t dev,
...
static bool ggml_backend_webgpu_device_supports_op(ggml_backend_dev_t dev,
  ...
  if (ggml_nbytes(op) > webgpu_ctx->limits.maxStorageBufferBindingSize ||
      (src0 != nullptr &&
       ggml_nbytes(src0) > webgpu_ctx->limits.maxStorageBufferBindingSize) ||
      (src1 != nullptr &&
       ggml_nbytes(src1) > webgpu_ctx->limits.maxStorageBufferBindingSize)) {
    return false;
  }
  ...
  if (ggml_nbytes(op) > webgpu_ctx->limits.maxStorageBufferBindingSize ||
      ...
      (src1 != nullptr &&
       ggml_nbytes(src1) > webgpu_ctx->limits.maxStorageBufferBindingSize) ||
      (src2 != nullptr &&
       ggml_nbytes(src2) > webgpu_ctx->limits.maxStorageBufferBindingSize)) {
    supports_op = false;
  ...
  if (!supports_op) {
    WEBGPU_LOG_DEBUG(
        "ggml_webgpu op not supported: "
        << ggml_op_name(op->op)
        << " with types dst: " << ggml_type_name(op->type) << ", src0: "
        << (op->src[0] ? ggml_type_name(op->src[0]->type) : "null")
        << ", src1: "
        << (op->src[1] ? ggml_type_name(op->src[1]->type) : "null"));
  } else {
    WEBGPU_LOG_DEBUG(
        "ggml_webgpu op supported: "
        << ggml_op_name(op->op)
        << " with types dst: " << ggml_type_name(op->type) << ", src0: "
        << (op->src[0] ? ggml_type_name(op->src[0]->type) : "null")
        << ", src1: "
...
static const char *ggml_backend_webgpu_reg_get_name(ggml_backend_reg_t reg) {
  ggml_backend_webgpu_reg_context *ctx =
      static_cast<ggml_backend_webgpu_reg_context *>(reg->context);
  return ctx->name;
}
...
// TODO: move most logic to device_init function so backend can be
// freed/initialized properly Only one device is supported for now
static ggml_backend_dev_t
ggml_backend_webgpu_reg_get_device(ggml_backend_reg_t reg, size_t index) {
  GGML_ASSERT(index == 0);
  WEBGPU_LOG_DEBUG("ggml_backend_reg_get_device()");

  WEBGPU_CPU_PROFILE_TOTAL_START(reg_get_device);

  ggml_backend_webgpu_reg_context *reg_ctx =
      static_cast<ggml_backend_webgpu_reg_context *>(reg->context);
  ...
  const char *const adapterEnabledToggles[] = {"vulkan_enable_f16_on_nvidia",
  ...
  ctx->instance.WaitAny(
      ctx->instance.RequestAdapter(
          ...
          [&ctx](wgpu::RequestAdapterStatus status, wgpu::Adapter adapter,
                 const char *message) {
  ...
  if (ctx->adapter.HasFeature(
    ...
    for (size_t i = 0; i < subgroup_matrix_configs.configCount; i++) {
      const wgpu::SubgroupMatrixConfig config =
          subgroup_matrix_configs.configs[i];
      if (config.M == config.N && config.N == config.K &&
          (config.K == 8 || config.K == 16) &&
          config.componentType == wgpu::SubgroupMatrixComponentType::F16 &&
          config.resultComponentType ==
              wgpu::SubgroupMatrixComponentType::F16) {
  ...
  dev_desc.SetDeviceLostCallback(wgpu::CallbackMode::AllowSpontaneous,
                                 [](const wgpu::Device &device,
                                   ...
                                   GGML_UNUSED(reason);
                                   GGML_UNUSED(message);
                                   // TODO: uncomment once proper free logic is
                                   // in place GGML_LOG_ERROR("ggml_webgpu:
                                   // Device lost! Reason: %d, Message: %s\n",
                                   // static_cast<int>(reason),
                                   // std::string(message).c_str());
                                 });
  dev_desc.SetUncapturedErrorCallback([](const wgpu::Device &device,
                                         wgpu::ErrorType reason,
                                         wgpu::StringView message) {
    GGML_UNUSED(device);
  ...
  const char *const deviceEnabledToggles[] = {
      "skip_validation", "disable_robustness", "disable_workgroup_init",
  ...
  ctx->instance.WaitAny(
      ctx->adapter.RequestDevice(
          ...
          [ctx](wgpu::RequestDeviceStatus status, wgpu::Device device,
                wgpu::StringView message) {
  ...
  ctx->param_buf_pool.init(
      ctx->device, WEBGPU_NUM_PARAM_BUFS, WEBGPU_PARAMS_BUF_SIZE_BYTES,
      wgpu::BufferUsage::CopyDst | wgpu::BufferUsage::Uniform,
      wgpu::BufferUsage::CopySrc | wgpu::BufferUsage::MapWrite);

#ifdef GGML_WEBGPU_GPU_PROFILE
  // Initialize buffer pool for timestamp queries (profiling)
  ctx->timestamp_query_buf_pool.init(
      ctx->device, WEBGPU_NUM_TIMESTAMP_QUERY_BUFS,
      WEBGPU_TIMESTAMP_QUERY_BUF_SIZE_BYTES,
      wgpu::BufferUsage::QueryResolve | wgpu::BufferUsage::CopySrc,
      wgpu::BufferUsage::MapRead | wgpu::BufferUsage::CopyDst);
#endif

  ctx->set_rows_error_buf_pool.init(
      ctx->device, WEBGPU_NUM_SET_ROWS_ERROR_BUFS,
  ...
  ggml_webgpu_create_buffer(ctx->device, ctx->debug_host_buf,
                            WEBGPU_DEBUG_BUF_ELEMS * sizeof(uint32_t),
                            wgpu::BufferUsage::CopyDst |
                                wgpu::BufferUsage::MapRead,
                            "debug_host_buf");
  ggml_webgpu_create_buffer(
      ctx->device, ctx->debug_dev_buf,
      WEBGPU_DEBUG_BUF_ELEMS * sizeof(uint32_t),
  ...
  GGML_LOG_INFO("ggml_webgpu: adapter_info: vendor_id: %u | vendor: %s | "
                "architecture: %s | device_id: %u | name: %s | "
                "device_desc: %s\n",
                info.vendorID, std::string(info.vendor).c_str(),
                std::string(info.architecture).c_str(), info.deviceID,
...
ggml_backend_reg_t ggml_backend_webgpu_reg() {
  ...
  if (webgpu_ctx->instance == nullptr) {
    GGML_LOG_ERROR("ggml_webgpu: Failed to create WebGPU instance. Make sure "
...
ggml_backend_t ggml_backend_webgpu_init(void) {
  ggml_backend_dev_t dev =
      ggml_backend_reg_dev_get(ggml_backend_webgpu_reg(), 0);
