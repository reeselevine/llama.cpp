...
using webgpu_encoded_batch = webgpu_command;
...
static void ggml_backend_webgpu_submit(webgpu_global_context & ctx, std::vector<webgpu_command> & commands) {
    GGML_UNUSED(ctx);
    GGML_UNUSED(commands);
}
...
static webgpu_encoded_batch ggml_backend_webgpu_build_multi(
...
static webgpu_encoded_batch ggml_backend_webgpu_build(webgpu_global_context &           ctx,
                                                      webgpu_param_arena &              param_arena,
                                                      wgpu::CommandEncoder &            encoder,
...
static webgpu_encoded_batch ggml_webgpu_cpy(webgpu_context & ctx, wgpu::CommandEncoder & encoder, ggml_tensor * src, ggml_tensor * dst) {
...
static webgpu_encoded_batch ggml_webgpu_set(webgpu_context & ctx, wgpu::CommandEncoder & encoder, ggml_tensor * src0, ggml_tensor * src1, ggml_tensor * dst) {
...
static webgpu_encoded_batch ggml_webgpu_pad(webgpu_context & ctx, wgpu::CommandEncoder & encoder, ggml_tensor * src, ggml_tensor * dst) {
    ggml_webgpu_shader_lib_context shader_lib_ctx = {
...
static webgpu_encoded_batch ggml_webgpu_solve_tri(webgpu_context & ctx,
                                                  wgpu::CommandEncoder & encoder,
                                                  ggml_tensor *    src0,
                                                  ggml_tensor *    src1,
                                                  ggml_tensor *    dst) {
...
static webgpu_encoded_batch ggml_webgpu_ssm_conv(webgpu_context & ctx,
                                                 wgpu::CommandEncoder & encoder,
                                                 ggml_tensor *    src0,
                                                 ggml_tensor *    src1,
                                                 ggml_tensor *    dst) {
...
static webgpu_encoded_batch ggml_webgpu_gated_delta_net(webgpu_context & ctx,
                                                        wgpu::CommandEncoder & encoder,
                                                        ggml_tensor *    src0,
                                                        ggml_tensor *    src1,
                                                        ggml_tensor *    src2,
...
static std::optional<webgpu_encoded_batch> ggml_webgpu_set_rows(webgpu_context & ctx,
                                                                wgpu::CommandEncoder & encoder,
                                                                ggml_tensor *    src,
                                                                ggml_tensor *    idx,
                                                                ggml_tensor *    dst) {
...
static webgpu_encoded_batch ggml_webgpu_get_rows(webgpu_context & ctx,
                                                 wgpu::CommandEncoder & encoder,
                                                 ggml_tensor *    src,
                                                 ggml_tensor *    idx,
                                                 ggml_tensor *    dst) {
...
static webgpu_encoded_batch ggml_webgpu_mul_mat(webgpu_context & ctx,
                                                wgpu::CommandEncoder & encoder,
                                                ggml_tensor *    src0,
                                                ggml_tensor *    src1,
                                                ggml_tensor *    dst) {
...
static webgpu_encoded_batch ggml_webgpu_flash_attn(webgpu_context & ctx,
                                                   wgpu::CommandEncoder & encoder,
                                                   ggml_tensor *    Q,
                                                   ggml_tensor *    K,
                                                   ggml_tensor *    V,
...
static webgpu_encoded_batch ggml_webgpu_unary_op(webgpu_context & ctx, wgpu::CommandEncoder & encoder, ggml_tensor * src, ggml_tensor * dst) {
...
static webgpu_encoded_batch ggml_webgpu_binary_op(webgpu_context & ctx,
                                                  wgpu::CommandEncoder & encoder,
                                                  ggml_tensor *    src0,
                                                  ggml_tensor *    src1,
                                                  ggml_tensor *    dst) {
...
static webgpu_encoded_batch ggml_webgpu_concat(webgpu_context & ctx,
                                               wgpu::CommandEncoder & encoder,
                                               ggml_tensor *    src0,
                                               ggml_tensor *    src1,
                                               ggml_tensor *    dst) {
...
static webgpu_encoded_batch ggml_webgpu_repeat(webgpu_context & ctx, wgpu::CommandEncoder & encoder, ggml_tensor * src0, ggml_tensor * dst) {
    uint32_t ne = (uint32_t) ggml_nelements(dst);
...
static webgpu_encoded_batch ggml_webgpu_row_norm(webgpu_context & ctx, wgpu::CommandEncoder & encoder, ggml_tensor * src, ggml_tensor * dst) {
    ...
    return ggml_backend_webgpu_build(ctx->global_ctx, ctx->param_arena, encoder, pipeline, params, entries, ggml_nrows(src));
}

static webgpu_encoded_batch ggml_webgpu_rope(webgpu_context & ctx,
                                             wgpu::CommandEncoder & encoder,
                                             ggml_tensor *    src0,
                                             ggml_tensor *    src1,
                                             ggml_tensor *    src2,
...
static webgpu_encoded_batch ggml_webgpu_glu(webgpu_context & ctx, wgpu::CommandEncoder & encoder, ggml_tensor * src0, ggml_tensor * src1, ggml_tensor * dst) {
    ggml_webgpu_shader_lib_context shader_lib_ctx = {
        .src0        = src0,
        .src1        = src1,
        .dst         = dst,
...
static webgpu_encoded_batch ggml_webgpu_scale(webgpu_context & ctx, wgpu::CommandEncoder & encoder, ggml_tensor * src, ggml_tensor * dst) {
    bool inplace = ggml_webgpu_tensor_equal(src, dst);
...
static webgpu_encoded_batch ggml_webgpu_soft_max(webgpu_context & ctx,
                                                 wgpu::CommandEncoder & encoder,
                                                 ggml_tensor *    src0,
                                                 ggml_tensor *    src1,
                                                 ggml_tensor *    src2,
    ...
    return ggml_backend_webgpu_build(ctx->global_ctx, ctx->param_arena, encoder, pipeline, params, entries, ggml_nrows(dst));
}

static webgpu_encoded_batch ggml_webgpu_argmax(webgpu_context & ctx, wgpu::CommandEncoder & encoder, ggml_tensor * src, ggml_tensor * dst) {
    std::vector<uint32_t> params = { (uint32_t) (ggml_webgpu_tensor_misalignment(ctx, src) / ggml_type_size(src->type)),
                                     (uint32_t) (ggml_webgpu_tensor_misalignment(ctx, dst) / ggml_type_size(dst->type)),
                                     (uint32_t) src->ne[0] };
...
static webgpu_encoded_batch ggml_webgpu_argsort(webgpu_context & ctx, wgpu::CommandEncoder & encoder, ggml_tensor * src, ggml_tensor * dst) {
    ...
    if (merge_passes == 0) {
        return ggml_backend_webgpu_build_multi(ctx->global_ctx, ctx->param_arena, encoder, pipelines, params_list, entries_list,
                                               workgroups_list);
    }
...
static webgpu_encoded_batch ggml_webgpu_cumsum(webgpu_context & ctx, wgpu::CommandEncoder & encoder, ggml_tensor * src, ggml_tensor * dst) {
    std::vector<uint32_t> params = { (uint32_t) (ggml_webgpu_tensor_misalignment(ctx, src) / ggml_type_size(src->type)),
                                     (uint32_t) (ggml_webgpu_tensor_misalignment(ctx, dst) / ggml_type_size(dst->type)),
                                     (uint32_t) src->ne[0] };
...
static webgpu_encoded_batch ggml_webgpu_sum_rows(webgpu_context & ctx, wgpu::CommandEncoder & encoder, ggml_tensor * src, ggml_tensor * dst) {
...
static std::optional<webgpu_encoded_batch> ggml_webgpu_encode_node(webgpu_context ctx, wgpu::CommandEncoder & encoder, ggml_tensor * node) {
    ...
    switch (node->op) {
        ...
        case GGML_OP_GATED_DELTA_NET:
            return ggml_webgpu_gated_delta_net(ctx, encoder, src0, src1, src2, node->src[3], node->src[4], node->src[5], node);
        case GGML_OP_PAD:
...
static ggml_status ggml_backend_webgpu_graph_compute(ggml_backend_t backend, struct ggml_cgraph * cgraph) {
    ...
    std::vector<webgpu_encoded_batch> commands;
#ifdef GGML_WEBGPU_GPU_PROFILE
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        if (num_batched_kernels >= WEBGPU_COMMAND_SUBMIT_BATCH_SIZE) {
            ...
            ggml_backend_webgpu_submit(ctx->global_ctx, commands);
#ifdef GGML_WEBGPU_GPU_PROFILE
