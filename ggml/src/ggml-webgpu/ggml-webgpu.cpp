...
static webgpu_command ggml_webgpu_get_rows(webgpu_context & ctx,
    ...
    webgpu_pipeline pipeline;
    {
        ...
        if (it != ctx->get_rows_pipelines.end()) {
            ...
            pipeline =
                ggml_webgpu_create_pipeline(ctx->device, processed.wgsl.c_str(), processed.variant.c_str(), constants);
            pipeline.context = processed.decisions;
...
static webgpu_command ggml_webgpu_mul_mat(webgpu_context & ctx,
            ...
            if (shader_src) {
                ...
                if (shader_lib_ctx.key.is_vec) {
                    ggml_webgpu_mul_mat_shader_decisions * decisions =
                        static_cast<ggml_webgpu_mul_mat_shader_decisions *>(processed.decisions);
    ...
    if (decisions.is_vec) {
        ...
        if (decisions.use_subgroup_matrix) {
            uint32_t wg_m_sg_tile = decisions.subgroup_m * decisions.subgroup_matrix_m * ctx->sg_mat_m;
            wg_m                  = CEIL_DIV(dst->ne[0], wg_m_sg_tile);
            uint32_t wg_n_sg_tile = decisions.subgroup_n * decisions.subgroup_matrix_n * ctx->sg_mat_n;
...
static webgpu_command ggml_webgpu_scale(webgpu_context & ctx, ggml_tensor * src, ggml_tensor * dst) {
    ...
    ggml_webgpu_scale_shader_lib_context shader_lib_ctx = { .key = key,
                                                            .max_wg_size =
                                                                ctx->limits.maxComputeInvocationsPerWorkgroup };
