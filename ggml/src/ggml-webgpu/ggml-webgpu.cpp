...
struct webgpu_context_struct {
    ...
    std::unordered_map<ggml_webgpu_mul_mat_pipeline_key, webgpu_pipeline, ggml_webgpu_mul_mat_pipeline_key_hash> mul_mat_pipelines; // src0_type, src1_type, vectorized
    ...
    std::unordered_map<ggml_webgpu_set_rows_pipeline_key, webgpu_pipeline, ggml_webgpu_set_rows_pipeline_key_hash>
                                                  set_rows_pipelines;
    std::unordered_map<ggml_webgpu_get_rows_pipeline_key, webgpu_pipeline, ggml_webgpu_get_rows_pipeline_key_hash> get_rows_pipelines; // src_type, vectorized
...
static webgpu_command ggml_webgpu_mul_mat(webgpu_context & ctx,
    ...
    ggml_webgpu_mul_mat_pipeline_key key = {
        ...
        .use_subgroup_matrix = (use_fast && !is_vec && ctx->supports_subgroup_matrix) ? 1 : 0,
        .register_tile = (use_fast && !is_vec && !ctx->supports_subgroup_matrix) ? 1 : 0
    };
    ...
    ggml_webgpu_mul_mat_shader_lib_context shader_lib_ctx = {
        ...
        .outputs_per_wg = static_cast<uint32_t>(WEBGPU_MUL_MAT_VEC_OUTPUTS_PER_WG)
    };
    ...
    
    {
        ...
        if (it != ctx->mul_mat_pipelines.end()) {
            ...
            if (!use_fast) {
                ...
                // These are the fallback for quantized types not supported by fast paths
            ...
            if (shader_src) {
                ...
                if (shader_lib_ctx.key.is_vec) {
                    printf("DEBUG: Creating mul_mat_vec pipeline - wg_size=%u, tile_k=%u, outputs_per_wg=%u\n",
                        shader_lib_ctx.wg_size, shader_lib_ctx.tile_k, shader_lib_ctx.outputs_per_wg);
                    ...
                    printf("DEBUG: Creating reg_tile pipeline - wg_size_m=%u, wg_size_n=%u, tile_k=%u\n",
                        shader_lib_ctx.wg_size_m, shader_lib_ctx.wg_size_n, shader_lib_ctx.tile_k);
                ...
                printf("DEBUG: Creating pipeline with variant='%s', constants.size()=%zu\n", 
